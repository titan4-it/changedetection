...
def apply_mask(image_path, mask_path, output_masked_path):
    with rasterio.open(image_path) as img_src, rasterio.open(mask_path) as mask_src:
        img_data = img_src.read(1).astype('float32')
        mask_data = mask_src.read(1).astype('float32')
        
        masked_img = np.where(mask_data == 1, img_data, np.nan).astype('float32')
        
        meta = img_src.meta
        meta.update(count=1, dtype='float32')
        
        with rasterio.open(output_masked_path, 'w', **meta) as dst:
            dst.write(masked_img, 1)

def compute_difference(masked_bu1_path, masked_bu2_path, output_diff_path, threshold=None):
    with rasterio.open(masked_bu1_path) as src1, rasterio.open(masked_bu2_path) as src2:
...
masked_bu1_path = 'masked_bu_output1.tif'
masked_bu2_path = 'masked_bu_output2.tif'
...
# Reproject the landcover image to match the BU images
reproject_to_match(landcover_image_path, output_bu1_path, reprojected_landcover_path)

# Create urban mask
mask_path = create_urban_mask(reprojected_landcover_path, urban_value=7)

# Apply urban mask to the BU images
apply_mask(output_bu1_path, mask_path, masked_bu1_path)
apply_mask(output_bu2_path, mask_path, masked_bu2_path)

# Compute the difference image using masked BU images
compute_difference(masked_bu1_path, masked_bu2_path, output_diff_path)
