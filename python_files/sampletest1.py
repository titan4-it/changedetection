...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun 20 10:34:14 2024
...
def compute_difference(image1_path, image2_path, output_diff_path, threshold=None):
    with rasterio.open(image1_path) as src1, rasterio.open(image2_path) as src2:
        ...
        # Apply the pixel value range condition [0, 1] and > 0
        mask1 = (bu1 > 0) & (bu1 <= 1)
        mask2 = (bu2 > 0) & (bu2 <= 1)
        valid_mask = mask1 & mask2
        
        # Compute the difference only for valid pixels
        diff = np.where(valid_mask, bu2 - bu1, np.nan)
        
        # Apply threshold if specified
        if threshold is not None:
            change_map = np.abs(diff) > threshold
            diff = np.where(change_map, diff, np.nan)
...
output1_path = 'S2A_MSIL1C_20230818T100031_N0509_R122_T33TTG_20230818T121434_resampled_bu_rome_NDBI.tif'
output2_path = 'S2A_MSIL1C_20240214T100121_N0510_R122_T33TTG_20240214T104957_resampled_bu_rome_NDBI.tif'
