...
def plot_pixel_values_details(pixel_values_image1, pixel_values_image2, coordinates):
    num_layers = len(pixel_values_image1[0])  # Assuming all rows have the same number of layers
    
    for coord_idx, (coord, pv_image1, pv_image2) in enumerate(zip(coordinates, pixel_values_image1, pixel_values_image2)):
        print(f'Pixel values for Coordinate {coord_idx + 1} (Lat: {coord[0]}, Lon: {coord[1]}):')
        print('Layer | Method | Image1 Value | Image2 Value')
        for layer_idx in range(num_layers):
            print(f'{layer_idx + 1} | {methods[layer_idx]} | {pv_image1[layer_idx]:.4f} | {pv_image2[layer_idx]:.4f}')
        print()
    
    # Create combined plots for all coordinates
    for coord_idx, coord in enumerate(coordinates):
        plt.figure(figsize=(14, 7))
        
        plt.plot(range(1, num_layers + 1), pixel_values_image1[coord_idx], marker='o', label='Image1')
        plt.plot(range(1, num_layers + 1), pixel_values_image2[coord_idx], marker='x', label='Image2')
        
        plt.title(f'Pixel Values at Coordinate {coord_idx + 1} (Lat: {coord[0]}, Lon: {coord[1]})')
        plt.xlabel('Layer Index')
        plt.ylabel('Pixel Value')
        plt.xticks(range(1, len(methods) + 1), methods, rotation=45)
        plt.grid(True)
...
plot_pixel_values_details(pixel_values_image1, pixel_values_image2, coordinates)
